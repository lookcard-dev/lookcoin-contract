...
import {
  ...
  testPausableFunction,
  assertBalanceChanges,
  assertSupplyChanges,
  expectSpecificRevert,
  testAllRevertScenarios,
  coverageTracker,
...
describe("ERC20Security - Comprehensive Security Tests", function () {
      ...
      it("should test all role combinations for minting", async function () {
        ...
        await testBooleanCombinations(
          ...
          async (combination) => {
            if (combination.to) {
              await fixture.lookCoin.grantRole(minterRole, fixture.user.address);
              await expect(fixture.lookCoin.connect(fixture.user).mint(fixture.user.address, amount))
                .to.not.be.reverted;
            } else {
  ...
  describe("Pause Mechanism Tests", function () {
    it("should test all pausable functions", async function () {
      ...
      const pausableFunctions = [
        {
          name: "transfer",
          operation: () => fixture.lookCoin.connect(fixture.user).transfer(fixture.user2.address, ethers.parseEther("10")),
        },
        {
          name: "transferFrom",
          operation: () => fixture.lookCoin.connect(fixture.user2).transferFrom(fixture.user.address, fixture.user2.address, ethers.parseEther("10")),
        },
        {
          name: "mint",
        ...
        {
          name: "approve",
          operation: () => fixture.lookCoin.connect(fixture.user).approve(fixture.user2.address, ethers.parseEther("10")),
        },
      ];
      ...
      for (const func of pausableFunctions) {
        ...
        await expectSpecificRevert(func.operation, 
          fixture.lookCoin, "Pausable: paused");
    ...
    it("should test pause state transitions", async function () {
      await testBooleanCombinations(
        ...
        async (combination) => {
          ...
          const operation = () => fixture.lookCoin.connect(fixture.user).transfer(fixture.user2.address, ethers.parseEther("1"));
  ...
  describe("Reentrancy Protection Tests", function () {
    ...
    beforeEach(async function () {
      ...
      const MaliciousContract = await ethers.getContractFactory("MockReentrantAttacker");
      maliciousContract = await MaliciousContract.deploy(await fixture.lookCoin.getAddress());
      
      // Grant necessary roles to malicious contract for testing
      await fixture.lookCoin.grantRole(await fixture.lookCoin.MINTER_ROLE(), await maliciousContract.getAddress());
      await fixture.lookCoin.grantRole(await fixture.lookCoin.BURNER_ROLE(), await maliciousContract.getAddress());
    });

    it("should prevent reentrancy on mint", async function () {
      await expectSpecificRevert(
        async () => maliciousContract.attackMint(fixture.user.address, ethers.parseEther("100")),
        fixture.lookCoin,
    ...
    it("should prevent reentrancy on burn", async function () {
      // First mint some tokens to the malicious contract
      await fixture.lookCoin.connect(fixture.minter).mint(await maliciousContract.getAddress(), ethers.parseEther("1000"));
      
      await expectSpecificRevert(
        async () => maliciousContract.attackBurn(await maliciousContract.getAddress(), ethers.parseEther("100")),
        fixture.lookCoin,
        "ReentrancyGuard: reentrant call"
      );
    ...
    describe("Approval and TransferFrom", function () {
      it("should approve and transferFrom correctly", async function () {
        ...
        await assertBalanceChanges(
          ...
          -amount,
          async () => {
            await fixture.lookCoin.connect(fixture.user2).transferFrom(fixture.user.address, fixture.user2.address, amount);
          }
        );
      ...
      it("should handle infinite approval correctly", async function () {
        ...
        await fixture.lookCoin.connect(fixture.user2).transferFrom(fixture.user.address, fixture.user2.address, transferAmount);
      ...
      it("should revert transferFrom with insufficient allowance", async function () {
        ...
        await expectSpecificRevert(
          async () => fixture.lookCoin.connect(fixture.user2).transferFrom(fixture.user.address, fixture.user2.address, amount),
          fixture.lookCoin,
      ...
      it("should test approval state transitions", async function () {
        ...
        for (let i = 0; i < amounts.length - 1; i++) {
          ...
          coverageTracker.trackBooleanCombination("LookCoin", `approval-${fromAmount > 0 ? "set" : "unset"}-to-${toAmount > 0 ? "set" : "unset"}`);
        }
      });
    });
  });
  ...
  describe("Supply Tracking Tests", function () {
    it("should track supply changes through all operations", async function () {
      const operations = [
        { type: "mint", amount: ethers.parseEther("1000"), expectedMintChange: ethers.parseEther("1000"), expectedBurnChange: BigInt(0) },
        { type: "burn", amount: ethers.parseEther("200"), expectedMintChange: BigInt(0), expectedBurnChange: ethers.parseEther("200") },
        { type: "mint", amount: ethers.parseEther("500"), expectedMintChange: ethers.parseEther("500"), expectedBurnChange: BigInt(0) },
        { type: "burn", amount: ethers.parseEther("300"), expectedMintChange: BigInt(0), expectedBurnChange: ethers.parseEther("300") },
      ];
      
      for (const op of operations) {
        await assertSupplyChanges(
          fixture.lookCoin,
          op.expectedMintChange,
          op.expectedBurnChange,
          async () => {
            if (op.type === "mint") {
  ...
  describe("Edge Cases and Boundary Tests", function () {
    it("should handle maximum uint256 values", async function () {
      ...
      await expect(
        fixture.lookCoin.connect(fixture.minter).mint(fixture.user.address, maxAmount)
      ).to.be.reverted;
    ...
    it("should handle zero amount edge cases", async function () {
      ...
      await expect(fixture.lookCoin.connect(fixture.user).transfer(fixture.user2.address, 0))
        .to.not.be.reverted;
