...
describe("IBCModule Test", function () {
  ...
  beforeEach(async function () {
    ...
    lookCoin = await upgrades.deployProxy(LookCoin, [governanceVault.address, ethers.ZeroAddress], {
  ...
  it("Should reject insufficient validators", async function () {
    ...
    await expect(ibcModule.connect(governanceVault).updateValidatorSet(validators, 2)).to.be.revertedWith(
      "IBC: insufficient validators",
    );
  });
