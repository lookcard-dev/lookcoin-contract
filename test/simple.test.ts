...
describe("Simple LookCoin Test", function () {
  ...
  it("Should enforce rate limiting", async function () {
    ...
    await expect(
      lookCoin.connect(addr1).mint(addr2.address, ethers.parseEther("1"))
  ...
  it("Should handle pause/unpause functionality", async function () {
    ...
    await expect(
      lookCoin.connect(addr1).mint(addr2.address, ethers.parseEther("100"))
    ).to.be.revertedWithCustomError(lookCoin, "EnforcedPause");
