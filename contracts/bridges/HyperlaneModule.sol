    ...
    IMessageRecipient
{
    ...
    function handle(
        ...
        // Decode message
        (address recipient, uint256 amount, bytes32 originalTransferId) = abi.decode(
            _message,
    ...
    function updateConfig(bytes calldata config) external override onlyRole(BRIDGE_ADMIN_ROLE) {
        ...
        if (gasAmount > 0) {
            setRequiredGasAmount(gasAmount);
        }
        if (ism != address(0)) {
            setInterchainSecurityModule(ism);
        }
        if (domain > 0 && trustedSender != address(0)) {
            setTrustedSender(domain, trustedSender);
        }
    }
